      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 pb-24 lg:pb-8 space-y-6">
        ...
        {feedback && (
          <div className={`rounded-xl px-4 py-3 text-sm font-medium flex items-center justify-between ${
            ...
            <span>{feedback}</span>
            <button onClick={() => setFeedback("")} className="text-slate-400 hover:text-muted-foreground">&times;</button>
          </div>
        )}
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div>
              ...
              <select
                ...
                className="h-10 rounded-xl border border-border bg-card px-3 text-sm font-medium text-muted-foreground focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 focus:outline-none min-w-[200px]"
              >
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                <Button variant="outline" size="icon-sm" onClick={() => cambiarFecha(-1)} className="rounded-lg border-border"><ChevronLeft className="h-4 w-4" /></Button>
                <input type="date" value={fecha} onChange={e => { setFecha(e.target.value); setDraft({}); }} className="h-10 rounded-xl border border-border bg-card px-3 text-sm focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 focus:outline-none" />
                <Button variant="outline" size="icon-sm" onClick={() => cambiarFecha(1)} className="rounded-lg border-border"><ChevronRight className="h-4 w-4" /></Button>
              </div>
            </div>
            <div className="flex items-end gap-2 ml-auto">
              <Button variant="outline" size="sm" onClick={marcarTodosPresente} className="h-10 gap-1 rounded-xl border-border"><Check className="h-4 w-4" />Todos presentes</Button>
        ...
        {cursoSeleccionado ? (
          <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm">
            <div className="px-5 py-4 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-violet-50">
              <h3 className="font-bold text-foreground text-sm">{cursoSeleccionado.nombre} · {fechaLegible(fecha)}</h3>
            </div>
            {asistencia.length === 0 ? (
              <div className="py-16 text-center">
                <p className="text-muted-foreground font-medium">No hay estudiantes en este curso</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-50">
                {asistencia.map((a) => (
                  <div key={a.studentId} className="flex flex-wrap items-center justify-between gap-2 p-4 hover:bg-muted transition-colors">
          ...
          <div className="bg-card rounded-2xl border border-border p-16 text-center shadow-sm">
            ...
            <p className="font-semibold text-muted-foreground">Selecciona un curso para comenzar</p>
          </div>
        )}
      </div>
    </div>
