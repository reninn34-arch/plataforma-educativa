...
export default function TeacherAsistenciaPage() {
  ...
  const toggleEstado = (studentId: number, currentEstado: string) => {
    ...
    setAsistencia(prev => prev.map(a =>
      a.studentId === studentId ? { ...a, estado: next } : a
    ));
  };
  ...
  const guardarAsistencia = async () => {
    ...
    try {
      const registros = asistencia
        ...
        .map(a => ({ studentId: a.studentId, estado: a.estado }));
  ...
  const marcarTodosPresente = () => {
    setAsistencia(prev => prev.map(a => ({ ...a, estado: "presente" })));
  };
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-4">
            <div>
              ...
              <select
                ...
                className="h-9 rounded-lg border border-input bg-card px-3 text-sm min-w-[200px]"
              >
                {cursos.map(c => (
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                <Button variant="outline" size="icon-sm" onClick={() => cambiarFecha(-1)}>
            ...
            <div className="flex items-end gap-2 ml-auto">
              <Button variant="outline" size="sm" onClick={marcarTodosPresente} className="h-9 gap-1">
                <Check className="h-4 w-4" /> Todos presentes
              </Button>
              <Button size="sm" onClick={guardarAsistencia} disabled={saving} className="h-9 gap-1">
          ...
          {conteo.total > 0 && (
            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t">
              ...
              {conteo.pendiente > 0 && (
                <Badge variant="outline" className="text-xs text-muted-foreground">{conteo.pendiente} sin marcar</Badge>
              )}
            </div>
          )}
        </CardContent>
      ...
      {loading ? (
        ...
        <Card className="shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle className="text-base">
              {cursoSeleccionado.nombre} · {fechaLegible(fecha)}
            </CardTitle>
          </CardHeader>
                    ...
                    <div className="flex items-center gap-1">
                      {ESTADOS.map(est => {
                        ...
                        return (
                          <button
                            key={est.value}
                            onClick={() => toggleEstado(a.studentId, a.estado)}
                            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                              isActive
                                ? `${est.color} text-white shadow-sm`
                                : `bg-muted/50 ${est.textColor} hover:bg-muted`
                            }`}
                            title={est.label}
                          >
        ...
        <Card className="shadow-sm">
          <CardContent className="py-16 text-center">
            <p className="text-muted-foreground">Selecciona un curso para comenzar</p>
          </CardContent>
        </Card>
      )}
    </div>
