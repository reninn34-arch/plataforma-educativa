          ...
          <form onSubmit={handleChangePin} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <input type={showPin ? "text" : "password"} value={currentPin} onChange={e => setCurrentPin(e.target.value.replace(/\D/g, "").slice(0, 4))} maxLength={4} className="w-full h-11 rounded-xl border border-border bg-card px-4 pr-11 text-sm font-bold tracking-[0.3em] placeholder:tracking-normal focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 focus:outline-none transition-all" placeholder="····" />
                <button type="button" onClick={() => setShowPin(!showPin)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-muted-foreground transition-colors">
