...
export function AccentColorPicker({ role = "student", userId }: { role?: string; userId?: number }) {
  ...
  useEffect(() => {
    ...
    // Sync the document attribute and trigger theme update to prevent stale theme leakage across sessions
    const currentAccent = document.documentElement.getAttribute("data-accent");
    if (currentAccent !== stored) {
  ...
  const handleSelect = (key: AccentColorKey) => {
    ...
    // Write specific cookies with 1 year max-age
    const maxAge = 31536000;
    if (userId) {
      document.cookie = `accent-color-user-${userId}=${key}; path=/; max-age=${maxAge}; SameSite=Lax`;
    }
    document.cookie = `accent-color-${role}=${key}; path=/; max-age=${maxAge}; SameSite=Lax`;
    document.cookie = `accent-color=${key}; path=/; max-age=${maxAge}; SameSite=Lax`;

    // Update the HTML data-accent attribute instantly
