  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <aside className="hidden lg:flex flex-col w-64 border-r bg-card fixed inset-y-0 z-40">
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
          {links.map(({ href, label, icon: Icon }) => {
            const isActive = pathname === href || (href !== `/${role}/dashboard` && pathname.startsWith(href));
            return (
              <Link
                key={href}
                href={href}
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:pl-64 pt-16 lg:pt-0">
        {/* Desktop Topbar for extra tools like notifications, profile etc. can go here if needed */}
        {children}
      </div>
    </div>
  );
